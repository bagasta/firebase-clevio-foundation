// ================================================================
// Pertemuan 4 — CRUD Lengkap: CREATE + READ + UPDATE + DELETE
// Konsep baru: deleteDoc, modal konfirmasi, statistik koleksi
// ================================================================

import { initializeApp }
    from "https://www.gstatic.com/firebasejs/11.0.0/firebase-app.js";
import { getFirestore, collection, addDoc, updateDoc, deleteDoc, doc, serverTimestamp, onSnapshot, query, orderBy }
...
function bukaModal(id, judul) {
    idYangDihapus = id;
    modalDesc.textContent = `"${judul}" akan dihapus permanen dari Firestore.`;
...
modalConfirm.addEventListener("click", async () => {
    if (!idYangDihapus) return;
    try {
        await deleteDoc(doc(db, COLLECTION, idYangDihapus));
        tampilkanToast("🗑️ Anime berhasil dihapus!");
    } catch (err) {
...
// ── ✨ BARU P4: Update Statistik ─────────────────────────────
function updateStats(docs) {
    const total = docs.length;
    const sudah = docs.filter(d => d.data().status === "Sudah Nonton").length;
    const ratings = docs.map(d => d.data().rating).filter(Boolean);
    const rataRata = ratings.length ? (ratings.reduce((a, b) => a + b, 0) / ratings.length).toFixed(1) : "-";

    document.getElementById("statTotal").textContent = total;
    document.getElementById("statSudah").textContent = sudah;
    document.getElementById("statRating").textContent = rataRata;
    animeCount.textContent = total;
}

// ── Render Kartu ─────────────────────────────────────────────
function buatKartu(snapDoc) {
    const { judul, genre, rating, status } = snapDoc.data();
    ...
    kartu.querySelector(".btn-edit").addEventListener("click", () => setModeEdit(snapDoc.id, snapDoc.data()));
    kartu.querySelector(".btn-delete").addEventListener("click", () => bukaModal(snapDoc.id, judul));
...
function dengarkanData() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        animeList.innerHTML = "";
        docs.forEach(d => animeList.appendChild(buatKartu(d)));
    });
}
